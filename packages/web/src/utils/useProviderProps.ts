...
import createAddressLookupTable from "./createAddressLookupTable";
import extendAddressLookupTable from "./extendAddressLookupTable";
import signAndSendTransactionV0WithLookupTable from "./signAndSendTransactionV0WithLookupTable";
...
export const useProviderProps = (): ProviderProps => {
  ...
  const handleSignAndSendTransactionV0WithLookupTable =
    useCallback(async () => {
      if (!provider || !provider.publicKey) return;
      try {
